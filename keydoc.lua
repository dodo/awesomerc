...
local function markup(keys)
   ...
   for _, key in ipairs(keys) do
     ...
     result[group] = (result[group] or "") ..
        '<span color="' .. beautiful.fg_normal .. '" font="DejaVu Sans Mono 7">' ..
        string.format("%" .. (longest - unilen(skey)) .. "s  ", "") .. skey ..
        '</span>  <span color="' .. beautiful.fg_urgent .. '">' ..
        help .. '</span>\n'
      end
   end
...
function display()
   ...
   for group, res in pairs(strings) do
      ...
      result = result ..
     '<span weight="bold" color="' .. beautiful.fg_minimize .. '">' ..
     group .. "</span>\n" .. res
   end
